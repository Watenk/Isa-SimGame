...
public class TileGrid : BaseClass
{
    ...
    public void SetTile(Vector2Int pos, ID groundID, ID airID, int temp, int lightLevel) 
    { 
        if (IsInGridBounds(pos))
        {
            Tile currentTile = GetTile(pos);
            currentTile.groundID = groundID;
            currentTile.airID = airID;
    ...
    private void InitializeGrid()
    {
        for (int y = 0; y < Height; y++)
        {
            ...
            {
                gridArray[x, y] = new Tile(ID.dirt, ID.carbonDioxite, new Vector2Int(x, y), 20000, 0);
            }
        }
    }
